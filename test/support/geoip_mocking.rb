...
# Deterministic GeoIP lookups for the Minitest suite.
#
# The RSpec suite stubs GeoIp.lookup for every example (spec/support/geoip_mocking.rb)
# so tests never depend on the real MaxMind database. The Minitest CI job has no
# MaxMind DB, so tests that resolve an IP to a country (compliance_blocked,
# ppp_details, tax location, etc.) would fail there while passing locally. This
# overrides GeoIp.lookup process-wide with the same fixed table.
#
# Keep the IP → location table in sync with spec/support/geoip_mocking.rb.
module GeoIpMocking
  DATA = {
    # Private/local IPs
    "127.0.0.1" => nil,
    ...
    "2.144.0.1" => { country: "Iran", code: "IR", region: "07", city: "Tehran", postal: "11369" },
...
# Override GeoIp.lookup for the whole Minitest process. Individual tests can
# still stub GeoIp::Result (e.g. an unrecognized country_code) on top of this.
GeoIp.singleton_class.prepend(Module.new do
  def lookup(ip)
    GeoIpMocking.result_for(ip)
  end
end)
