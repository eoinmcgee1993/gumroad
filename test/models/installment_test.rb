...
<p>Great deals!</p>
  ...
  private
    # Route URL helpers, without mixing them into the test class (see the note
    # at the top). Call as `routes.checkout_url(...)`.
    def routes
    ...
    # Deactivate then restart a subscription, recording the events delivery_due?
    # reads to decide whether a workflow post is due after a resubscribe.
    ...
    # A distinct IP per purchase within a test (see create_purchase).
    def unique_ip
      @ip_seq ||= 0
      @ip_seq += 1
    ...
    # Mirrors the :installment factories. Product/variant posts hang off a
    # product; seller/follower/affiliate/audience posts hang off a seller with
    # no link. Defaults follow whichever the type needs, and any of them can be
    # overridden.
    ...
    # Mirrors the base :purchase factory (see test/fixtures/purchases.yml for
    # the same money math): a successful sale on the platform Stripe account.
    # calculate_fees is an after(:build) hook in the factory, so we invoke it
    # explicitly before saving. variant_attributes/purchaser are assigned the
    # way callers pass them to the factory.
    def create_purchase(link:, seller: :default, purchaser: nil, variant_attributes: nil, **attrs)
      ...
      purchase = Purchase.new({
        ...
        # A unique IP per purchase mirrors the factory's random IP: not_double_charged
        # keys on email + ip_address + link, so a shared/nil IP makes two sales to the
        # same buyer look like a duplicate charge.
    ...
    # Subscriptions must have a payment_option before they validate, so build
    # it (priced at the product's default price) before saving, like the
    # :subscription factory's before(:create) hook does.
    ...
    # An original subscription sale. delivery_due? only cares about the
    # subscription and its events, so a plain (non-tiered) recurring product is
    # enough — no membership tiers needed.
