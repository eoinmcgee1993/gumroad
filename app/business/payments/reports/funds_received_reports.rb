...
module FundsReceivedReports
  def self.funds_received_report(month, year)
    ...
    refunded_purchase_ids = Refund.effective.where(created_at: range).where(purchase_id: successful_purchases).pluck(:purchase_id)
    ...
    json["Purchases"] = payment_methods.map do |name, purchases|
      ...
      partially_refunded_purchases = purchases.second
      {
        ...
        "Sales" => {
          ...
          total_transaction_cents: successful_or_partially_refunded_purchases.sum(:total_transaction_cents) - partially_refunded_purchases.joins(:effective_refunds).sum("refunds.total_transaction_cents"),
          gumroad_tax_cents: successful_or_partially_refunded_purchases.sum(:gumroad_tax_cents) - partially_refunded_purchases.joins(:effective_refunds).sum("refunds.gumroad_tax_cents"),
          affiliate_credit_cents: successful_or_partially_refunded_purchases.sum(:affiliate_credit_cents) - partially_refunded_purchases.joins(:effective_refunds).sum("TRUNCATE(purchases.affiliate_credit_cents * (refunds.amount_cents / purchases.price_cents), 0)"),
          fee_cents: successful_or_partially_refunded_purchases.sum(:fee_cents) - partially_refunded_purchases.joins(:effective_refunds).sum("refunds.fee_cents")
        }
      }
    end
