...
describe PostResendApi, :freeze_time do
  ...
  describe "text direction" do
    ...
    it "marks each top-level text block with dir=auto so mixed-language posts resolve direction per paragraph" do
      @post.update!(message: "<p>English intro</p><p>שלום עולם</p>")
      send_default_email

      paragraphs = html_doc(sent_email_content).css("div.rich-text > p")
  ...
  describe "Update reason" do
    ...
    it "avoids 'purchased' wording when installment_type='seller' and the recipient's purchase was free" do
      ...
      expect(sent_email_content).to include("You've received this email because you got a product from #{@post.seller.name}.")
    end
